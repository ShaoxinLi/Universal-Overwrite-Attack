...
from src.models.hidden import Encoder, Decoder, Discriminator
...
from src.watermarkings.hidden import HiDDeN, ScalarTracker, TensorBoardImageLogger, \
...
def prepare_model(args):
    ...
    args.logger(f"===========================> Loading HiDDeN network")
    args.encoder = Encoder(
        H=args.img_size, W=args.img_size, encoder_channels=64, encoder_blocks=4, message_length=args.message_length
    )
    ...
    args.discriminator = Discriminator(discriminator_channels=64, discriminator_blocks=3)
    ...
    if args.verbose:
        ...
        args.logger(f"===========================> Discriminator :\n {args.discriminator}")
        args.logger(f"# parameters: {get_num_parameters(args.discriminator)}")
...
if __name__ == "__main__":
    ...
    # instantiate a HiDDeN
    hidden = HiDDeN(device=args.device, seed=args.seed)
    ...
    hidden.train(
        encoder=args.encoder, decoder=args.decoder, noiser=args.noiser, discriminator=args.discriminator,
        loader=args.train_loader, message_length=args.message_length, num_epochs=args.num_epochs,
    ...
    hidden.test(
        encoder=args.encoder, decoder=args.decoder, noiser=args.noiser, discriminator=args.discriminator,
        loader=args.test_loader, gt_messages=args.test_gt_messages, num_eval_users=args.num_eval_users,
        num_total_users=args.num_total_users, ckpt_file_path=args.ckpt_file_path, callbacks=callbacks
    )
    args.logger(f"Ground truth message set at testing: {hidden.state.message_set.tolist()}")
    for metric_name, meter in hidden.state.eval_metric_meters.items():
