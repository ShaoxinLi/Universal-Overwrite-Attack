...
class GaussianBlur(torch.nn.Module):
    ...
    def forward(self, encoded_images):
        if not torch.is_tensor(encoded_images):
            raise TypeError("Input x type is not a torch.Tensor. Got {}".format(type(encoded_images)))
        if not len(encoded_images.shape) == 4:
            raise ValueError("Invalid input shape, we expect BxCxHxW. Got: {}".format(encoded_images.shape))

        # prepare kernel
        std = random.uniform(self.std_min, self.std_max)
        sigma = (std, std)
        self._padding: Tuple[int, int] = self.compute_zero_padding(self.kernel_size)
        self.kernel: torch.Tensor = get_gaussian_kernel2d(self.kernel_size, sigma)
        b, c, h, w = encoded_images.shape
        tmp_kernel: torch.Tensor = self.kernel.to(encoded_images.device).to(encoded_images.dtype)
        ...
        noised_images = F.conv2d(encoded_images, kernel, padding=self._padding, stride=1, groups=c)
