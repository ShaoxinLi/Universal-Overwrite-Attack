...
class DiffWMAttacker(WMAttacker):
    def __init__(self, pipe, batch_size=20, noise_step=60, std=1.0, captions={}):
        self.pipe = pipe
    ...
    def attack(self, image_paths, out_paths, return_latents=False, return_dist=False):
        with torch.no_grad():
            ...
            for (img_path, out_path), prompt in zip(zip(image_paths, out_paths), prompts):
                ...
                if self.std > 0:
                    noise = torch.normal(mean=0.0, std=self.std, size=[1, 4, img.shape[-2] // 8, img.shape[-1] // 8], device=self.device)
                else:
                    noise = torch.zeros([1, 4, img.shape[-2] // 8, img.shape[-1] // 8], device=self.device)
                if return_dist:
