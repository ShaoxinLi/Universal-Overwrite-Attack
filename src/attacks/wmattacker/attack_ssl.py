...
# load carriers
carrier_path = "./Datasets/coco10k/ssl/carriers/carrier_30_2048.pth"
...
def test(logger, std=1.0, num_total_users=10, num_tests=10):
    ...
    for test_idx in range(num_tests):
        ...
        for i, (org_images, _) in enumerate(org_dataloader):
        ...
        wm_dataloader = torch.utils.data.DataLoader(
            dataset=wm_dataset, batch_size=16, shuffle=False,
            num_workers=4 * torch.cuda.device_count(), pin_memory=False
        )

        counter = 0
        for i, (watermarked_images, _) in enumerate(wm_dataloader):
        ...
        # attack
        watermarked_image_paths = list_files(watermarked_image_folder_path)
        watermarked_image_paths = sorted(watermarked_image_paths)
        temp_attacked_image_folder_path = f"./Datasets/imagenet10k/ssl_watermarked/test_n{num_total_users}/c{test_idx + 1}_wmattacked/0"
