...
class UOA(object):
    ...
    def train(self, decoder, loader, img_size, message_length, num_epochs, lr, xi, callbacks):

        self.decoder = decoder.to(self.device, non_blocking=True)
        self.decoder.eval()
        self.uoa = torch.zeros(1, 3, img_size, img_size, device=self.device, dtype=torch.float32)
        self.uoa.requires_grad_(True)

        self.optimizer = torch.optim.AdamW([self.uoa], lr=lr)
        ...
        for epoch in range(num_epochs):
            ...
            self.state.state_dict = {
                "uoa": self.uoa.data.detach(), "optimizer": self.optimizer,
            }
    ...
    def _train_epoch(self, loader):
        ...
        for i, (images, messages) in enumerate(loader):
            ...
            self._train_step(images, messages)
    ...
    def _train_step(self, images, messages):
        ...
        # project
        self.uoa.data = self.uoa.data * min(1, self.xi / (torch.linalg.norm(self.uoa.data)))
        ...
        self.state.uoa = (self.uoa.data.detach() - self.uoa.data.detach().min()) / (self.uoa.data.detach().max() - self.uoa.data.detach().min())
    ...
    def _eval_epoch(self, loader):
        ...
        uoa = copy.deepcopy(self.uoa.detach())
        for i, (images, messages) in enumerate(loader):
            self.state.iteration = i
            ...
            self.callbacks.on_batch_start()
            self._eval_step(images, messages, uoa)
            self.callbacks.on_batch_end()

        self.callbacks.on_loader_end()

    @torch.no_grad()
    def _eval_step(self, images, messages, uoa):

        # generate adv images
        attack_start_time = int(round(time.time() * 1000))
        adv_images = images + uoa
        adv_images = torch.clamp(adv_images, 0., 1.)
        ...
        self.state.uoa = (uoa - uoa.min()) / (uoa.max() - uoa.min())
        self.state.images = images
    ...
    def test(self, decoder, loader, xi, ckpt_file_path, callbacks):
        ...
        ckptio = CheckpointIO(
            ckpt_file_path=ckpt_file_path, device=self.device, uoa=self.uoa,
        )
        self.uoa = ckptio.load()["uoa"]
...
class TensorBoardImageLogger(Callback):
    ...
    def on_epoch_end(self):
        if self.state.is_train:
            ...
            adv_images = self.state.adv_images[:self.num_images]
            uoa = self.state.uoa

            if self.resize_to is not None:
                images = F.interpolate(images, size=self.resize_to)
                adv_images = F.interpolate(adv_images, size=self.resize_to)
                uoa = F.interpolate(uoa, size=self.resize_to)

            all_images = torch.cat([images, adv_images], dim=0)
            grid = torchvision.utils.make_grid(all_images, nrow=self.num_images)
            uoa = uoa.squeeze(0)

            self.writer.add_image("Images (Watermarked v.s. Adv)/train", grid, self.state.epoch + 1)
            self.writer.add_image("UOA/train", uoa, self.state.epoch + 1)
        else:
            uoa = self.state.uoa
            if self.resize_to is not None:
                uoa = F.interpolate(uoa, size=self.resize_to)
            uoa = uoa.squeeze(0)
            self.writer.add_image("UOA/test", uoa)

    @torch.no_grad()
    def on_batch_end(self):
        if not self.state.is_train:
            image = self.state.images
            adv_images = self.state.adv_images
            if self.resize_to is not None:
                image = F.interpolate(image, size=self.resize_to)
                adv_images = F.interpolate(adv_images, size=self.resize_to)

            all_images = torch.cat([image, adv_images], dim=0)
            grid = torchvision.utils.make_grid(all_images, nrow=all_images.size(0))
            self.writer.add_image("Images (Watermarked v.s. Adv)/test", grid, self.state.image_idx + 1)
